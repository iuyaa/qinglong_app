...
沿用上游的面板登录、多账号切换、任务、日志、环境变量、配置文件、脚本、订阅及依赖管理。现阶段先建立可构建基线，不代表所有功能已通过当前青龙版本的实测。

## 下载 APK

打开 [GitHub Actions](https://github.com/iuyaa/qinglong_app/actions/workflows/android.yml)，选择成功的 `Android APK` 构建，在 Artifacts 中下载 `qinglong-android-<构建号>`，解压安装 `qinglong-android.apk`。下载 Actions 产物需要登录 GitHub；产物保留 14 天，可手动重新构建。

- 新包名：`io.github.iuyaa.qinglong`，可与原 APK 同时安装，账号配置独立。
- Android 5.0 及以上是当前构建配置的最低要求；实际手机兼容性待验证。
- 使用本仓库专用签名，同一签名的后续构建可覆盖升级。
- 初始版本为 3.0.1，版本代码使用 Actions 构建号。

## 云端构建

代码推送到 `main` 或手动运行工作流时，GitHub 托管的 Ubuntu runner 自动安装 Java/Flutter、解析锁定依赖、执行响应处理测试、构建并验签 APK。
...
- 真实面板和 Android 真机功能验证仍需单独进行；构建成功不等于功能验收。
